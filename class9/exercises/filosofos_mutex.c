...
/* 0 - garfo em uso            */ 
/* 1 - garfo desocupado        */ 
int garfos[lenThread]={1,1,1,1,1}; 
 
/* mutex_locks para cada garfo*/ 
pthread_mutex_t m[lenThread];  
 
/* ID Filosofos */ 
int p[lenThread]={0,1,2,3,4}; 
 
/* numero de refeicoes consumidadas pelos filosofos */ 
int numRefeicoes[lenThread]={0,0,0,0,0}; 
int numTentativas[lenThread]={0,0,0,0,0}; 
...
{  
    ...
    { 
        ...
        { /* the left is available */ 
            ...
            pthread_mutex_lock( m + ((sub+1)%lenThread) ); 
             
            if( garfos[(sub+1)%lenThread]==1 ) 
            { /* the right is available */ 
                printf("Filosofo %d: Direita=%d\n", sub,
                                    garfos[(sub+1)%lenThread]);
                
                /* set the right chopstick unavailable */ 
                garfos[(sub+1)%lenThread]=0; 
                 
                /* unlock the right chopstick */ 
                pthread_mutex_unlock( m + ( (sub+1) % lenThread) ); 
